...
if(verticalMode){
[scrollView setContentSize:CGSizeMake(scrollView.bounds.size.width*2, self.bounds.size.height)];
}else{
...
if(scrollView.contentOffset.x >= 0 && scrollView.contentOffset.x <= self.bounds.size.width*0.3) {
...
} else if (scrollView.contentOffset.x < 0 && scrollView.contentOffset.x > -35) {
...
[[%c(LSApplicationWorkspace) defaultWorkspace] openApplicationWithBundleID:self.bundleID];
});
}
...
SBApplication* currentApp = [[objc_getClass("SpringBoard") sharedApplication] _accessibilityTopDisplay];
