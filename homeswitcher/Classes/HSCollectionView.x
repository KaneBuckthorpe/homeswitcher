...
notify_register_dispatch("com.kaneb.HomeSwitcher/HSCollectionViewPrefChanged", &regToken, dispatch_get_main_queue(), ^(int token) {
[self loadPrefs];
...
NSArray*selectedArray = [preferences objectForKey:@"selectedApps"];
self.blurAppsList = [selectedArray valueForKey:@"bundleID"];
...
cell.blurView.hidden=YES;
    ...
    LSApplicationProxy *appProxy = [%c(LSApplicationProxy) applicationProxyForIdentifier:bundleID];
...
UIColor* leastCommon = [[self mainColoursInImage: cell.iconImageView.image detail:0] lastObject];
...
if (self.verticalMode){
[self setContentOffset:CGPointMake(0, cellHeight*floor(([self.switcherItems count]-1)/2)) animated:NO];
}else{
...
if (self.verticalMode){
               [self setContentOffset:CGPointMake(0, self.contentSize.height-viewAreaHeight) animated:NO];
} else{
               [self setContentOffset:CGPointMake(self.contentSize.width-viewAreaWidth, 0) animated:NO];
}
}
