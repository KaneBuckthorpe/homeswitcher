...
- (void)initWithPrefs {
    ...
    int regToken;
    notify_register_dispatch("com.kaneb.HomeSwitcher/HSLoadPrefs", &regToken,
                             dispatch_get_main_queue(), ^(int token) {
                                 [self loadPrefs];
                             });
    notify_register_dispatch("com.kaneb.HomeSwitcher/HSPrefChanged", &regToken,
                             dispatch_get_main_queue(), ^(int token) {
                                 [self loadPrefs];
                                 [self loadHomeView];
    ...
    notify_register_dispatch("com.kaneb.HomeSwitcher/showOrHide", &regToken,
                             dispatch_get_main_queue(), ^(int token) {
...
- (void)loadPrefs {
    ...
    self.activatorGesturesEnabled =
    [preferences boolForKey:@"activatorGesturesEnabled"];
    
    overlapStatusBar = [preferences boolForKey:@"overlapStatusBar"];
    
    self.gestureViewThickness =
    100 * ([preferences floatForKey:@"gestureViewThickness"] / 100);
...
- (void)loadSwitcherWindow {
    if (!self.switcherWindow) {
        self.switcherWindow = [[HSWindow alloc]
...
- (void)loadHomeView {
    ...
    self.homeView = [[HSHomeView alloc]
                     initWithFrame:CGRectMake(UIScreen.mainScreen.bounds.size.width *
                                              self.pageSliderNumber,
                                              self.homeViewOriginY,
                                              UIScreen.mainScreen.bounds.size.width,
                                              UIScreen.mainScreen.bounds.size.height * 0.8)];
    ...
    self.topViewSB = [[objc_getClass("SBIconController") sharedInstance]
...
- (void)loadSwitcherView {
    ...
    switcherCenter =
    CGPointFromString([preferences objectForKey:@"switcherViewPosition"]);
...
- (void)loadGestureViews {
    ...
    rightArrow = [[UIView alloc]
                  initWithFrame:CGRectMake(switcherOriginX + switcherWidth -
                                           self.gestureViewThickness,
                                           switcherOriginY, self.gestureViewThickness,
                                           switcherHeight)];
    
    leftArrow = [[UIView alloc]
                 initWithFrame:CGRectMake(switcherOriginX, switcherOriginY,
                                          self.gestureViewThickness, switcherHeight)];
    
    centerButton = [[UIView alloc]
                    initWithFrame:CGRectMake(switcherOriginX + self.gestureViewThickness,
                                             switcherOriginY + switcherHeight -
                                             self.gestureViewThickness,
                                             switcherWidth -
                                             (self.gestureViewThickness * 2),
                                             self.gestureViewThickness)];
    ...
    UITapGestureRecognizer *scrollToCenterTap = [[UITapGestureRecognizer alloc]
                                                 initWithTarget:self
                                                 action:@selector(scrollToPosition:)];
    ...
    UITapGestureRecognizer *scrollToStartTap = [[UITapGestureRecognizer alloc]
                                                initWithTarget:self
                                                action:@selector(scrollToPosition:)];
    ...
    UITapGestureRecognizer *scrollToEndTap = [[UITapGestureRecognizer alloc]
                                              initWithTarget:self
                                              action:@selector(scrollToPosition:)];
    ...
    UIPanGestureRecognizer *centerPan = [[UIPanGestureRecognizer alloc]
                                         initWithTarget:self
                                         action:@selector(moveSwitcher:)];
...
- (void)moveSwitcher:(UIPanGestureRecognizer *)pan {
    ...
    if ((switcherCenter.y >= self.switcherView.frame.size.height / 2 &&
         switcherCenter.y <= self.homeView.frame.size.height -
         self.switcherView.frame.size.height / 2 &&
         !self.switcherView.verticalMode) ||
        (switcherCenter.x >= self.switcherView.frame.size.width / 2 &&
         switcherCenter.x <= self.homeView.frame.size.width -
         self.switcherView.frame.size.width / 2 &&
         self.switcherView.verticalMode)) {
            ...
            [preferences setObject:NSStringFromCGPoint(switcherCenter)
                            forKey:@"switcherViewPosition"];
        }
...
(UIGestureRecognizer *)otherGestureRecognizer {
    ...
    if (([[gestureRecognizer view] isDescendantOfView:self.homeView]) &&
        ![[otherGestureRecognizer view]
          isDescendantOfView:[gestureRecognizer view]]) {
            return YES;
        } else {
            return NO;
        }
