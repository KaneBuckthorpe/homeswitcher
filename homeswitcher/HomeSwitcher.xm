...
- (void)frontDisplayDidChange:(id)newDisplay {
    ...
    if (newDisplay == nil &&
        ...
        [homeSwitcher hideSwitcher];
                ...
                isDescendantOfView:homeSwitcher.topViewSB] ||
               [homeSwitcher.homeView
                isDescendantOfView:homeSwitcher.switcherWindow]) {
                   
                   [homeSwitcher.homeView.superview
                    bringSubviewToFront:homeSwitcher.homeView];
    ...
    if (homeSwitcher.activatorGesturesEnabled) {
        if ([listenerName isEqualToString:@"com.indiedevkb.hideSwitcher"]) {
            ...
            [homeSwitcher hideSwitcher];
                    ...
                    isEqualToString:@"com.indiedevkb.showSwitcher"]) {
            [homeSwitcher showSwitcher];
            
        } else if ([listenerName
                    isEqualToString:@"com.indiedevkb.toggleSwitcher"]) {
...
+ (void)load {
    ...
    [activator registerListener:[self new]
                        forName:@"com.indiedevkb.hideSwitcher"];
