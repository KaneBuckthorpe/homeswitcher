...
- (void)layoutSubviews {
    ...
    if ([platform isEqualToString:@"developer"]) {
        ...
        self.textLabel.font = [UIFont fontWithName:@"HelveticaNeue-Medium"
                                              size:20.0];
        self.textLabel.textAlignment = 1;
    } else {
        float logoWidth = 40;
        float labelOriginX = (screenWidth / 2) -
        ((labelWidth + logoWidth + 5) / 2) + 5 + logoWidth;
        
        float logoOriginX = labelOriginX - 5 - logoWidth;
        
        logoFrame = CGRectMake(logoOriginX, 10, 30, 30);
        
        labelFrame = CGRectMake(labelOriginX, 10,
                                screenWidth - logoFrame.size.width + 20,
                                logoFrame.size.height);
        self.textLabel.font = [UIFont fontWithName:@"HelveticaNeue-Light"
                                              size:30.0];
    ...
    if ([platform isEqualToString:@"paypal"]) {
        ...
        self.textLabel.textColor = [UIColor colorWithRed:0
                                                   green:45.0f / 255.0f
                                                    blue:139.0f / 255.0f
                                                   alpha:1];
    } else if ([platform isEqualToString:@"twitter"]) {
        self.textLabel.textColor = [UIColor colorWithRed:(29.0f / 255.0f)
                                                   green:(161.0f / 255.0f)
                                                    blue:(242.0f / 255.0f)
                                                   alpha:1];
    }
