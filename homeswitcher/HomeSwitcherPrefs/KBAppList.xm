...
[toggleAllButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
toggleAllButton.backgroundColor=UIColor.grayColor;
[toggleAllButton addTarget:self 
action:@selector(toggleAllPressed)
forControlEvents:UIControlEventTouchUpInside];
[toggleAllButton setTitle:@"Select/deselect all" forState:UIControlStateNormal];
...
if(cell == nil)
{
cell = [[KBAppListCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:@"CELL"];
}
cell.appName.text = [self.installedApps objectAtIndex:indexPath.row];
cell.appIconView.image = [UIImage _applicationIconImageForBundleIdentifier: [self.installedAppsBundleID  objectAtIndex:indexPath.row] format:1 scale:UIScreen.mainScreen.scale];

if([self.selectedApps containsObject:[self.installedAppsBundleID objectAtIndex:indexPath.row]]) 
{
cell.appSelected=YES;
cell.checkBoxView.image=[UIImage imageNamed: @"hide" inBundle:[NSBundle bundleWithPath:@"/Library/PreferenceBundles/HomeSwitcher.bundle"] compatibleWithTraitCollection:nil];
}
else
{
cell.appSelected=NO;
...
{
saveButton.tintColor=[UIColor colorWithRed:1 green:0.17 blue:0.33 alpha:1];
...
if(!cell.appSelected)
{
[self.selectedApps addObject:[self.installedAppsBundleID objectAtIndex:indexPath.row]];
...
if([self.selectedApps containsObject:[self. installedAppsBundleID objectAtIndex:indexPath.row]])
{
[self.selectedApps removeObject:[self. installedAppsBundleID objectAtIndex:indexPath.row]]; 
...
if(!self.allSelected){
[toggleAllButton setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
NSMutableArray*holding= [NSMutableArray arrayWithArray:self. installedAppsBundleID];
///add to array
...
CFNotificationCenterPostNotification(r, (CFStringRef)@"com.kaneb.HomeSwitcher/HSCollectionViewPrefChanged", NULL, NULL, true);
